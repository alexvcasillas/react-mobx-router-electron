...
This project is a personal boilerplate as a quickstart for development with [react-create-app](https://github.com/facebookincubator/create-react-app) + [react-router](https://github.com/ReactTraining/react-router) and [MobX](https://github.com/mobxjs/mobx) to maintain the state of the App and [Electron](https://github.com/electron/electron) to build an awesome desktop application.
...
To build your desktop application you only need to run `yarn dist`. This will _execute react-create-app_ build command and after that, it will start the build process of electron and generate a `dist` folder that will contain an installer of your app and also the `unpacked` version of your app if you don't want to distribute it with an installer. And that's it, you're ready to go!
